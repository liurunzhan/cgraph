...
void cgraph_verilog_clkgen(FILE *fp, const cgraph_size_t len)
{
    if ((NULL != fp) && (!ferror(fp))) {
        cgraph_size_t i, max = cgraph_math_pow2(len), max_1 = max - 1,
                         len_1 = len - 1;
        fprintf(fp,
                "module clkgen (" CGRAPH_PLAT_NLINE "   rstn," CGRAPH_PLAT_NLINE
        ...
        fprintf(fp,
                "output reg         clko;" CGRAPH_PLAT_NLINE CGRAPH_PLAT_NLINE);
...
void cgraph_verilog_sync(FILE *fp, const cgraph_size_t len)
{
    if (NULL != fp && !ferror(fp)) {
        ...
        fprintf(fp, "module sync (" CGRAPH_PLAT_NLINE
                    "  rstn," CGRAPH_PLAT_NLINE "  clki," CGRAPH_PLAT_NLINE
                    "  signi," CGRAPH_PLAT_NLINE "  signo" CGRAPH_PLAT_NLINE
        ...
        fprintf(fp,
                "always @(posedge clki or negedge rstn)" CGRAPH_PLAT_NLINE
                "  begin" CGRAPH_PLAT_NLINE "    if(!rstn)" CGRAPH_PLAT_NLINE);
        fprintf(fp, "       signs[%ld:0] <= {%ld{1'b0}};" CGRAPH_PLAT_NLINE,
                len_1, len);
        fprintf(fp,
                "    else" CGRAPH_PLAT_NLINE "      begin" CGRAPH_PLAT_NLINE);
        fprintf(
            fp,
...
void cgraph_verilog_filter(FILE *fp, const cgraph_size_t len)
{
    if (NULL != fp && !ferror(fp)) {
        ...
        fprintf(fp,
                "reg reg_signo;" CGRAPH_PLAT_NLINE
                "always @(posedge clki or negedge rstn)" CGRAPH_PLAT_NLINE
                "  begin" CGRAPH_PLAT_NLINE "    if(!rstn)" CGRAPH_PLAT_NLINE
        ...
        fprintf(fp,
                "        reg_signo <= 1'b1;" CGRAPH_PLAT_NLINE
                "  end" CGRAPH_PLAT_NLINE CGRAPH_PLAT_NLINE
...
void cgraph_verilog_simple(FILE *fp, const cgraph_size_t len)
{
    if (NULL != fp && !ferror(fp)) {
        ...
        fprintf(fp, "reg [%ld:0] signs;" CGRAPH_PLAT_NLINE, len_1);
        fprintf(fp,
                "reg         sign_last;" CGRAPH_PLAT_NLINE CGRAPH_PLAT_NLINE
                "always @(posedge clki or negedge rstn)" CGRAPH_PLAT_NLINE
                "  begin" CGRAPH_PLAT_NLINE "    if(!rstn)" CGRAPH_PLAT_NLINE);
        fprintf(fp, "       signs[%ld:0] <= {%ld{1'b0}};" CGRAPH_PLAT_NLINE,
                len_1, len);
        ...
        fprintf(
            ...
            len_1, len - 2);
        fprintf(fp,
                "      end" CGRAPH_PLAT_NLINE
                "  end" CGRAPH_PLAT_NLINE CGRAPH_PLAT_NLINE
                "always @(posedge clki or negedge rstn)" CGRAPH_PLAT_NLINE
                "  begin" CGRAPH_PLAT_NLINE "    if(!rstn)" CGRAPH_PLAT_NLINE
                "       sign_last <= 1'b0;" CGRAPH_PLAT_NLINE);
        fprintf(fp,
                "    else if(signs[%ld:0] == {%ld{1'b0}})" CGRAPH_PLAT_NLINE,
                len_1, len);
        fprintf(fp, "       sign_last <= 1'b0;" CGRAPH_PLAT_NLINE);
        fprintf(fp,
                "    else if(signs[%ld:0] == {%ld{1'b1}})" CGRAPH_PLAT_NLINE,
                len_1, len);
...
{
    if (NULL != fp && !ferror(fp)) {
        fprintf(fp, "module edgedect (" CGRAPH_PLAT_NLINE
                    "  rstn," CGRAPH_PLAT_NLINE "  clki," CGRAPH_PLAT_NLINE
                    "  signi," CGRAPH_PLAT_NLINE "  signp," CGRAPH_PLAT_NLINE
                    "  signn" CGRAPH_PLAT_NLINE ");" CGRAPH_PLAT_NLINE
                    "input  rstn;" CGRAPH_PLAT_NLINE
                    "input  clki;" CGRAPH_PLAT_NLINE
                    "input  signi;" CGRAPH_PLAT_NLINE
                         ...
                         const cgraph_size_t len)
{
    if (NULL != fp && !ferror(fp)) {
        ...
        fprintf(fp, "module fifo (" CGRAPH_PLAT_NLINE
                    "  rstn," CGRAPH_PLAT_NLINE "  clk," CGRAPH_PLAT_NLINE
                    "  write," CGRAPH_PLAT_NLINE "  read," CGRAPH_PLAT_NLINE
                    "  big," CGRAPH_PLAT_NLINE "  din," CGRAPH_PLAT_NLINE
                    "  dout" CGRAPH_PLAT_NLINE ");" CGRAPH_PLAT_NLINE);
        fprintf(fp, "input  rstn;" CGRAPH_PLAT_NLINE
                    "input  clki;" CGRAPH_PLAT_NLINE
                    "input  write;" CGRAPH_PLAT_NLINE
                    "input  read;" CGRAPH_PLAT_NLINE
...
void cgraph_verilog_shift(FILE *fp, const cgraph_size_t len)
{
    if (NULL != fp && !ferror(fp)) {
        ...
        fprintf(fp, "module shift (" CGRAPH_PLAT_NLINE
                    "  rstn," CGRAPH_PLAT_NLINE "  clk," CGRAPH_PLAT_NLINE
                    "  write," CGRAPH_PLAT_NLINE "  read," CGRAPH_PLAT_NLINE
                    "  msb," CGRAPH_PLAT_NLINE "  din," CGRAPH_PLAT_NLINE
                    "  dout" CGRAPH_PLAT_NLINE ");" CGRAPH_PLAT_NLINE);
        fprintf(fp, "input  rstn;" CGRAPH_PLAT_NLINE
                    "input  clki;" CGRAPH_PLAT_NLINE
                    "input  write;" CGRAPH_PLAT_NLINE
        ...
        fprintf(fp,
                ...
                "  begin" CGRAPH_PLAT_NLINE "    if(!rstn)" CGRAPH_PLAT_NLINE);
        fprintf(fp,
                "        reg_shift[%ld:0] <= {%ld{1'b0}};" CGRAPH_PLAT_NLINE,
                len_1, len);
        fprintf(fp, "    else if(write)" CGRAPH_PLAT_NLINE
                    "      begin" CGRAPH_PLAT_NLINE
                    "        if(msb)" CGRAPH_PLAT_NLINE);
        fprintf(fp,
                "            reg_shift[%ld:0] <= din[%ld:0];" CGRAPH_PLAT_NLINE,
                len_1, len_1);
        fprintf(fp, "        else" CGRAPH_PLAT_NLINE);
...
void cgraph_verilog_tbench(FILE *fp, const cgraph_size_t delay_time)
{
    fprintf(fp, "`timescale 1ns/1ps" CGRAPH_PLAT_NLINE CGRAPH_PLAT_NLINE
                "module tbench();" CGRAPH_PLAT_NLINE
                "reg clk;" CGRAPH_PLAT_NLINE "reg rstn;" CGRAPH_PLAT_NLINE);
    fprintf(fp,
            "always @(*)" CGRAPH_PLAT_NLINE "  begin" CGRAPH_PLAT_NLINE
